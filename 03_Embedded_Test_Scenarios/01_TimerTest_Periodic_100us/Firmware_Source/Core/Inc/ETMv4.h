...
/*
   ETMv4 Register Definition
   Based on ARM IHI 0064H Chapter 7
*/
#include <stdint.h>
...
// Assuming TPIU_BASE is defined as the base address of your TPIU peripheral
#define TPIU_BASE       ((uint32_t)0x5C015000) // Standard CoreSight TPIU base address
#define TPIU            ((TPIU_TypeDef *) TPIU_BASE)
/* Trace Funnel (CSTF) Base Address (D3 Domain) */
#define CSTF_BASE 0x5C013000
#define CSTF ((CSTF_TypeDef *) CSTF_BASE)
/* Embedded Trace FIFO (ETF/TMC) Base Address (D3 Domain) */
...
/* Timestamp Generator (TSG) - feeds the 64-bit global timestamp that ETM's
 * TS packets carry. VERIFIED base address from RM0433 Table 604 (System ROM
 * table 1): component base address (system bus) = 0x5C005000. Without this
 * enabled, TRCCONFIGR.TS still makes the ETM emit I_TIMESTAMP packets, but
 * every one carries whatever the (never-started) counter holds - i.e. 0. */
...
typedef struct {
    ...
    /* Read-only silicon ID - copy into ETM_REGS in TempDecoder_V0.2.py */
    ...
    uint32_t tsg_cntcr, tsg_cntsr, tsg_cvl, tsg_cvu;   // != 0/increasing -> TSG really running
} Trace_Status_t;
...
extern Trace_Status_t g_trace_status;
typedef struct {
...
typedef struct {
    ...
    uint32_t RESERVED_5[3];         // 0xFBC - 0xFC4 (3 words - replacing DEVARCH with reserved)
    ...
    uint32_t RESERVED_6[3];         // 0xFD4 - 0xFDC (3 words - PDF explicitly notes these are Reserved)
    ...
    volatile uint32_t CIDR3;        // 0xFFC: Component ID Register 3 [cite: 3, 60]
} TPIU_TypeDef;
...
typedef struct {
    ...
    /* Standard CoreSight Management Registers */
    volatile uint32_t CLAIMSET;     // 0xFA0: Claim Tag Set Register
    volatile uint32_t CLAIMCLR;     // 0xFA4: Claim Tag Clear Register
...
/**
 * @brief Serial Wire Output (SWO) Register Map
 * Based on STMicroelectronics RM0433 Reference Manual
 */
typedef struct {
    uint32_t RESERVED0[4];      // 0x000 to 0x00F: Reserved
    __IO uint32_t CODR;         // 0x010: SWO Clock Output Divider Register
    uint32_t RESERVED1[55];     // 0x014 to 0x0EC: Reserved
    __IO uint32_t SPPR;         // 0x0F0: SWO Selected Pin Protocol Register
    uint32_t RESERVED2[131];    // 0x0F4 to 0x2FC: Reserved
    __I  uint32_t FFSR;         // 0x300: SWO Formatter and Flush Status Register
    uint32_t RESERVED3[806];    // 0x304 to 0xF9C: Reserved
    __IO uint32_t CLAIMSET;     // 0xFA0: SWO Claim Tag Set Register
    __IO uint32_t CLAIMCLR;     // 0xFA4: SWO Claim Tag Clear Register
    uint32_t RESERVED4[2];      // 0xFA8 to 0xFAC: Reserved
    __O  uint32_t LAR;          // 0xFB0: SWO Lock Access Register
    __I  uint32_t LSR;          // 0xFB4: SWO Lock Status Register
    __I  uint32_t AUTHSTAT;     // 0xFB8: SWO Authentication Status Register
    uint32_t RESERVED5[3];      // 0xFBC to 0xFC4: Reserved
    __I  uint32_t DEVID;        // 0xFC8: SWO Device Configuration Register
    __I  uint32_t DEVTYPE;      // 0xFCC: SWO Device Type Identifier Register
    __I  uint32_t PIDR4;        // 0xFD0: SWO Peripheral ID Register 4
    uint32_t RESERVED6[3];      // 0xFD4 to 0xFDC: Reserved
    __I  uint32_t PIDR0;        // 0xFE0: SWO Peripheral ID Register 0
    __I  uint32_t PIDR1;        // 0xFE4: SWO Peripheral ID Register 1
    __I  uint32_t PIDR2;        // 0xFE8: SWO Peripheral ID Register 2
    __I  uint32_t PIDR3;        // 0xFEC: SWO Peripheral ID Register 3
    __I  uint32_t CIDR0;        // 0xFF0: SWO Component ID Register 0
    __I  uint32_t CIDR1;        // 0xFF4: SWO Component ID Register 1
    __I  uint32_t CIDR2;        // 0xFF8: SWO Component ID Register 2
    __I  uint32_t CIDR3;        // 0xFFC: SWO Component ID Register 3
} SWO_TypeDef;
#include <stdint.h>

/**
 * @brief Serial Wire Trace Flinger (SWTF) Register Map
 * Based on STMicroelectronics RM0433 Reference Manual
 */
typedef struct {
    __IO uint32_t CTRL;         // 0x000: SWTF Control Register
    __IO uint32_t PRIORITY;     // 0x004: SWTF Priority Register
    uint32_t RESERVED0[998];    // 0x008 to 0xF9C: Reserved
    __IO uint32_t CLAIMSET;     // 0xFA0: SWTF Claim Tag Set Register
    __IO uint32_t CLAIMCLR;     // 0xFA4: SWTF Claim Tag Clear Register
    uint32_t RESERVED1[2];      // 0xFA8 to 0xFAC: Reserved
    __O  uint32_t LAR;          // 0xFB0: SWTF Lock Access Register
    __I  uint32_t LSR;          // 0xFB4: SWTF Lock Status Register
    __I  uint32_t AUTHSTAT;     // 0xFB8: SWTF Authentication Status Register
    uint32_t RESERVED2[3];      // 0xFBC to 0xFC4: Reserved
    __I  uint32_t DEVID;        // 0xFC8: SWTF Device Configuration Register
    __I  uint32_t DEVTYPE;      // 0xFCC: SWTF Device Type Identifier Register
    __I  uint32_t PIDR4;        // 0xFD0: SWTF Peripheral ID Register 4
    uint32_t RESERVED3[3];      // 0xFD4 to 0xFDC: Reserved
    __I  uint32_t PIDR0;        // 0xFE0: SWTF Peripheral ID Register 0
    __I  uint32_t PIDR1;        // 0xFE4: SWTF Peripheral ID Register 1
    __I  uint32_t PIDR2;        // 0xFE8: SWTF Peripheral ID Register 2
    __I  uint32_t PIDR3;        // 0xFEC: SWTF Peripheral ID Register 3
    __I  uint32_t CIDR0;        // 0xFF0: SWTF Component ID Register 0
    __I  uint32_t CIDR1;        // 0xFF4: SWTF Component ID Register 1
    __I  uint32_t CIDR2;        // 0xFF8: SWTF Component ID Register 2
    __I  uint32_t CIDR3;        // 0xFFC: SWTF Component ID Register 3
} SWTF_TypeDef;

/**
 * @brief Timestamp Generator (TSG) Register Map
 * ARM CoreSight generic timestamp-counter interface (same shape as the
 * Armv8-A Generic Timer CNTControlBase frame). Feeds the 64-bit global
 * timestamp distributed to trace sources (ETM TS packets).
 */
typedef struct {
    __IO uint32_t CNTCR;        // 0x000: Counter Control Register - bit0 EN
    __I  uint32_t CNTSR;        // 0x004: Counter Status Register
    __I  uint32_t CNTCVL;       // 0x008: Counter Count Value, lower 32 bits
    __I  uint32_t CNTCVU;       // 0x00C: Counter Count Value, upper 32 bits
    uint32_t RESERVED0[4];      // 0x010 to 0x01C: Reserved
    __IO uint32_t CNTFID0;      // 0x020: Counter Frequency ID Register 0
    uint32_t RESERVED1[990];    // 0x024 to 0xF9C: Reserved
    __IO uint32_t CLAIMSET;     // 0xFA0: Claim Tag Set Register
    __IO uint32_t CLAIMCLR;     // 0xFA4: Claim Tag Clear Register
    uint32_t RESERVED2[2];      // 0xFA8 to 0xFAC: Reserved
    __O  uint32_t LAR;          // 0xFB0: Lock Access Register
    __I  uint32_t LSR;          // 0xFB4: Lock Status Register
    __I  uint32_t AUTHSTAT;     // 0xFB8: Authentication Status Register
    uint32_t RESERVED3[3];      // 0xFBC to 0xFC4: Reserved
    __I  uint32_t DEVID;        // 0xFC8: Device Configuration Register
    __I  uint32_t DEVTYPE;      // 0xFCC: Device Type Identifier Register
    __I  uint32_t PIDR4;        // 0xFD0: Peripheral ID Register 4
    uint32_t RESERVED4[3];      // 0xFD4 to 0xFDC: Reserved
    __I  uint32_t PIDR0;        // 0xFE0: Peripheral ID Register 0
    __I  uint32_t PIDR1;        // 0xFE4: Peripheral ID Register 1
    __I  uint32_t PIDR2;        // 0xFE8: Peripheral ID Register 2
    __I  uint32_t PIDR3;        // 0xFEC: Peripheral ID Register 3
    __I  uint32_t CIDR0;        // 0xFF0: Component ID Register 0
    __I  uint32_t CIDR1;        // 0xFF4: Component ID Register 1
    __I  uint32_t CIDR2;        // 0xFF8: Component ID Register 2
    __I  uint32_t CIDR3;        // 0xFFC: Component ID Register 3
} TSG_TypeDef;
