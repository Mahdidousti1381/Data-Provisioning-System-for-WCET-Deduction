...
function getExceptionName(excepNumHex) {
    ...
    if (Number.isNaN(num))
        return 'Exception';
    if (num < 0x20)
        return M_EXCEPTION_NAMES[num];
    if (num >= 0x208 && num <= 0x3EF)
        return `IRQ${num - 0x200}`;
    return 'Reserved';
}
/**
...
function vectorNameToNum(name) {
    ...
    const aliases = { pereset: 'reset', svc: 'svcall' };
    const target = aliases[key] || key;
    // Skip the IRQ0-7 slots (handled above) and the Reserved entries.
    const idx = M_EXCEPTION_NAMES.findIndex((x, i) => (i < 0x10 || i >= 0x18)
        && x !== 'Reserved'
...
class ReportGenerator {
    static async generateReportFromPpl(pplFilePath, snapshotPath, workspaceRoot, coreFreqMhz = 1, tsFreqMhz = 1) {
        ...
        const inDeadRange = (i) => i !== null && deadRanges.some(r => i >= r.from && i <= r.to);
        const synthesiseRaw = (raw, closure, note) => {
        ...
        if (rawExceptions.length > 0) {
            ...
            const rawEvents = [
                ...rawExceptions.map(r => ({ idx: r.traceIdx, kind: 'exc', raw: r })),
                ...rawRetIdx.map(i => ({ idx: i, kind: 'ret' }))
            ].sort((a, b) => a.idx - b.idx);
            const rawClosure = new Map();
            const rawStack = [];
            for (const ev of rawEvents) {
                if (ev.kind === 'exc') {
                    ...
                    if (f)
                        rawClosure.set(f.traceIdx, { closure: 'returned', note: null });
                }
            }
            ...
            const decodedByIdx = new Map();
            for (const r of allExceptions) {
                if (r.traceIdx !== null)
                    decodedByIdx.set(r.traceIdx, r);
            }
            merged = [];
            for (const raw of rawExceptions) {
                ...
                if (dec) {
                    ...
                    if (!dec.prefRetAddr)
                        dec.prefRetAddr = raw.prefRetAddr;
                    ...
                    rawOnlyExceptions++;
                    const rc = rawClosure.get(raw.traceIdx)
                        || { closure: 'no-return-eot', note: null };
            ...
            for (const rest of decodedByIdx.values())
                merged.push(rest);
        ...
        if (allExceptions.length <= 15) {
            ...
            if (allExceptions[mid + 1])
                picked.add(allExceptions[mid + 1]);
    ...
    static deadSpanFor(traceIdx, ranges) {
        const own = ranges.find(r => r.from === traceIdx);
        if (own)
            return `byte ${own.to.toLocaleString()} (${(own.to - own.from).toLocaleString()} bytes lost)`;
    ...
    <div class="container">
        ...
        <div class="card"${d.captureHealthy ? '' : ' style="border-color: rgba(248,81,73,0.6);"'}>
            ...
            <table>
                ...
                <tbody>
                    ${d.desyncEvents.map(ev => `<tr><td><code>${ev.traceIdx.toLocaleString()}</code></td><td><code>${ev.reason}</code></td><td>${this.deadSpanFor(ev.traceIdx, d.deadRanges)}</td></tr>`).join('')}
                </tbody>
            </table>`}
